...
#= 
As explained before, it is important for Julia to know the types. In this case, it is not possible to know the type of x and y, or even the return type. Hence, Julia creates many different versions of the function for each possible type. We can specify the types of the arguments and the return type of the function.
=#
...
# note that if the return type is not the one specified, it will give an error. This is very important for Julia since it knows the output type
...
#= 
Since we know that this is a function for scalar, we may be tempted to define by specifying the abstract type higher in the hierarchy of number, which is Real
=#
Term.typestree(Int)
...
# another solution is to define a new function that operated with vectors
...
# * #SUBSECTION: @time
#=
we can easily time the computational time of a function, and how many objects it creates
=#
